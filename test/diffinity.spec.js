...
describe('diffinity', function () {
		...
		it('initializes without arguments', (done) => {
			...
			editor.once('updated', () => {
				try {
					...
					expect(items[0] == 'diffinity-margin');
					expect(items[1] == 'diffinity-column');
					...
					expect(items[3] == 'diffinity-margin');
					expect(items[4] == 'diffinity-column');
					expect(items[5] == 'CodeMirror cm-s-default');
					expect(items[6] == 'diffinity-column');
					expect(items[7] == 'diffinity-splash');
		...
		it('should ignore white-space', function (done) {
			...
			editor.once('updated', () => {
				try {
					...
					expect(editor.el.querySelectorAll('.diffinity.rhs.c')).to.have.length(0);
					expect(editor.el.querySelectorAll('.diffinity.lhs.c')).to.have.length(0);
					done();
				} catch (ex) {
					done(ex);
				}
	...
	describe('unmarkup', () => {
		it('should remove markup', (done) => {
			...
			const test = () => {
				try {
					const found = document.querySelector('.diffinity.ch.ind.lhs');
