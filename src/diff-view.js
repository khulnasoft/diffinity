...
CodeMirrorDiffView.prototype.setOptions = function(opts) {
	...
	if (this.editor) {
		if (opts.hasOwnProperty('sidebar')) {
			const divs = document.querySelectorAll(`#${this.id} .diffinity-margin`);
		...
		if (opts.hasOwnProperty('rhs_margin') && opts.rhs_margin !== current_margin_rhs) {
			...
			const divs = document.querySelectorAll(`#${this.id} .diffinity-editor > div`);
...
CodeMirrorDiffView.prototype.bind = function(container) {
	...
	const el = dom.getDiffinityContainer({ clazz: container.className });
	...
	if (!lhstx) {
		console.error('lhs textarea not defined - Diffinity not initialized properly');
	}
	if (!rhstx) {
		console.error('rhs textarea not defined - Diffinity not initialized properly');
	}
	this._current_diff = -1;
