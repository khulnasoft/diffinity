...
class VDoc {
	...
	addRender(side, change, changeId, options) {
		...
		const bgClass = [ 'diffinity', side, `cid-${changeId}` ];
		...
		if (isCurrent) {
			...
			for (let j = lf; j <= lt; ++j) {
				this._getLine(side, j).addLineClass('gutter', 'diffinity current');
			}
		}
		...
		for (let j = lf, k = olf; lf !== -1 && lt !== -1 && j <= lt; ++j, ++k) {
			...
			if (side === 'lhs' && (change.op === 'd')) {
				...
				this._getLine(side, j).markText(0, undefined, `diffinity ch d lhs cid-${changeId}`);
				...
				this._getLine(side, j).markText(0, undefined, `diffinity ch a rhs cid-${changeId}`);
			}
		}
		...
		for (let j = lf, k = olf;
			...
			if (j <= lt && k <= olt) {
				...
				for (const change of results.changes()) {
					...
					lhs_line.markText(lhs_start, lhs_to, `diffinity ch ind lhs cid-${changeId}`);
					const rhs_line = vdoc._getLine('rhs', k);
					rhs_line.markText(rhs_start, rhs_to, `diffinity ch ina rhs cid-${changeId}`);
				}
			} else if (k > olt) {
				// lhs has exceeded the max lines in the rhs editor, remainder are deleted
				const line = vdoc._getLine('lhs', j);
				line.markText(0, undefined, `diffinity ch ind lhs cid-${changeId}`);
			} else if (j > lt) {
				// rhs has exceeded the max lines in the lhs editor, remainder are added
				const line = vdoc._getLine('rhs', k);
				line.markText(0, undefined, `diffinity ch ina rhs cid-${changeId}`);
			}
		}
	}
	...
	update(editor) {
		...
		editor.operation(() => {
			...
			if (this.markup.length) {
				// while Diffinity diffs unicode chars (letters+mark), CM is by character,
